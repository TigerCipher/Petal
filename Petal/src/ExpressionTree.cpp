    ...
    m_value{ std::move(value) }, m_children{ std::move(children) }, m_line_number{ line_number }, m_char_index{ char_index }
{
    ...
    std::visit(
        overloaded{ [&]([[maybe_unused]] const std::string& val) {
                    ...
                    [&](const node_operation val) {
                        ...
                        case node_operation::index:
                            ...
                            {
                                throw error::semantic(to_string(m_children[0]->type_id()) + " is not index-able", m_line_number,
                                                      m_char_index);
                            }
                            break;
                        ...
                        case node_operation::call:
                            if (const function_type * ft{ std::get_if<function_type>(m_children[0]->type_id()) })
                            {
                                ...
                                {
                                    throw error::semantic(std::format("Incorrect number of arguments. Expected {}, given {}",
                                                                      ft->parameter_type_id.size(), m_children.size() - 1),
                                                          m_line_number, m_char_index);
                                }
                                for (u32 i{ 0 }; i < ft->parameter_type_id.size(); ++i)
                                {
                                    if (m_children[i + 1]->lvalue() && !ft->parameter_type_id[i].by_ref)
                                    {
                                        throw error::semantic("Function does not receive the argument by reference",
                                                              m_children[i + 1]->line_number(), m_children[i + 1]->char_index());
                                    }
                            ...
                            {
                                throw error::semantic(std::format("{} is not callable", m_children[0]->type_id()), m_line_number,
                                                      m_char_index);
                            }
                            break;
                        }
...
void node::check_conversion(type_handle type_id, bool lvalue) const
{
    ...
    {
        throw error::incorrect_type(std::to_string(m_type_id), std::to_string(type_id), lvalue, m_line_number, m_char_index);
    }
}
